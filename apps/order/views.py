...
"""
    前端传递参数：地址(addr_id)，支付方式(pay_method)，用户要购买的是商品id字符串(sku_ids)
    mysql事务：一组sql语句，要么都失败，要么都成功
    并发：秒杀
    悲观锁与乐观锁的使用场景：
    乐观锁更适合冲突少，耗时少的，乐观锁重复操作的代价比较大
    悲观锁：强制加锁，所以适合冲突比较明显的场景
"""
# /order/commit
...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        # 初始化
        alipay = AliPay(
            appid="2016101900724274",  # 2016101900724274应用id
    ...
    def post(self, request):
        ...
        while True:
            ...
            if code == '10000' and response.get('trade_status') == 'TRADE_SUCCESS':
                ...
                order.save()
                # 返回结果
                return JsonResponse({'res': 3, 'message': '订单支付成功'})
            elif code == '40004' or (code == '10000' and response.get('trade_status') == 'WAIT_BUYER_PAY'):
                # 等待支付结果， 可能一会儿就支付成功，所以做一次等待，循环遍历
                import time
                time.sleep(5)
                continue
            else:
...
class CommentView(View):
    ...
    def post(self, request, order_id):
        ...
        for i in range(1, total_count + 1):
            ...
            # 获取评论区内评论内容
            context = request.POST.get('context_%d' % i, '')  # context_1 context_2
